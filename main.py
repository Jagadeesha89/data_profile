...
if __name__ == "__main__":
    try:
        ...
        data_path="D:\python_packages\Jan_23_rev - Copy.csv"
        logging.info("Reading data Started")
        df=pd.read_csv(data_path)
        data_sum=DataReader(df,y="Status",drop_cols=["Acc","Status"],cat_col_chart="countplot",num_col_chart="histplot",columns=4,figsize=(14,7))
        df_1=data_sum.data_summary()
        print(df_1)
        logging.info("Data summary completed")
        data_sum.data_charting()
        df2=data_sum.stats_summary() 
        print(df2)   
